...
do
  ...
  timeout "$timer" ./uina > /dev/null 2>&1 ; if [ $? -eq 124 ] ; then echo "$timer Initialized Name Variable" ; else echo Failed ; fi
  x=$(( $x + 1 ))
  rm -rf config.json 
  sleep 1
  ...
  timeout "$timers" ./uina > /dev/null 2>&1 ; if [ $? -eq 124 ] ; then echo "$epoch epochs , $timers " ; else echo Failed ; fi
  rm -rf config.json 
done
